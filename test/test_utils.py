...
def test_varargs():
    ...
    def f(x, *args):
        assert get_args(sys._getframe()) == OrderedDict((('x', x), ('*args', args)))
...
def test_kwargs():
    ...
    def f(x, **kwargs):
        assert get_args(sys._getframe()) == OrderedDict((('x', x), ('**kwargs', kwargs)))
...
def test_method():
    ...
    class cls(object):
        ...
        def f(self, a, b=2, *args, **kwargs):
            assert get_args(sys._getframe()) == OrderedDict(
                (('self', self), ('a', a), ('b', b), ('*args', args), ('**kwargs', kwargs))
            )
