# *-* coding: utf-8 *-*
from ._compat import (
    loads, dumps, JSONEncoder, quote, execute, u, StringIO, escape, from_bytes, force_bytes, is_str,
    _detect_lines_encoding, logger
)
from .utils import (
    get_source, get_doc, executable_line, importable_module, Html5Diff, search_key_in_obj, search_value_in_obj,
    timeout_of, inplace
)
...
def tokenize_redir(raw_data):
    ...
    for token_type, token, src, erc, line in generate_tokens(raw_io.readline):
        if (token_type == tokens.ERRORTOKEN and token == '!' and last_token in ('>', '>>')):
            return (line[:src[1] - 1], line[erc[1]:].lstrip(), last_token == '>>')
        last_token = token
    return
...
class Interaction(object):
    ...
    hooks = {'update_watchers': ['start', 'eval', 'watch', 'init', 'select', 'unwatch']}
    ...
    def init(self):
        self.db.send('Title|%s' % dump({'title': self.exception, 'subtitle': self.exception_description}))
        if self.shell:
            self.db.send('Shell')
        else:
            self.db.send('Trace|%s' % dump({'trace': self.trace, 'cwd': os.getcwd()}))
            self.db.send('SelectCheck|%s' % dump({'frame': self.current, 'name': self.current_file}))
        ...
        while not stop:
            ...
            try:
                ...
                try:
                    ...
                    self.db.send('Echo|%s' % dump({'for': 'Error in Wdb, this is bad', 'val': exc + '<br>' + link}))
                except Exception:
    ...
    def update_watchers(self):
        ...
        for watcher in self.db.watchers[self.current_file]:
            try:
                watched[watcher] = self.db.safe_better_repr(eval_(watcher, self.get_globals(), self.current_locals))
    ...
    def notify_exc(self, msg):
        ...
        self.db.send('Log|%s' % dump({'message': '%s\n%s' % (msg, traceback.format_exc())}))
    ...
    def do_file(self, data):
        ...
        self.db.send('Select|%s' % dump({'frame': self.current, 'name': fn, 'file': file}))
    ...
    def do_inspect(self, data):
        ...
        if (isinstance(thing, tuple) and len(thing) == 3):
            ...
            self.db.interaction(iter_tb.tb_frame, tb, type_.__name__, str(value))
            return
                ...
                try:
                    ...
                    if m:
                        ...
                        if self.db._importmagic_index:
                            scores = self.db._importmagic_index.symbol_scores(name)
        ...
        if redir and not self.db.hooked:
            ...
            self.db.send(
                'Print|%s' % dump(
                    {
                        'for': raw_data,
                        'result': escape('%s to file %s' % ('Appended' if append else 'Written', redir),)
                    }
                )
            )
        else:
            ...
            self.db.send('Print|%s' % dump({'for': raw_data, 'result': result, 'duration': duration}))
            if imports:
    ...
    def do_break(self, data):
        ...
        break_fail = lambda x: self.fail('Break', 'Break on %s failed' % ('%s:%s' % (brk['fn'], brk['lno'])), message=x)
        ...
        if brk['lno'] is not None:
            try:
                ...
                break_fail('Wrong breakpoint format must be ' '[file][:lineno][#function][,condition].')
                return

            line = getline(brk['fn'], lno, self.current_frame.f_globals)
            if not line:
                for path in sys.path:
                    line = getline(os.path.join(path, brk['fn']), brk['lno'], self.current_frame.f_globals)
                    if line:
                        break
            if not line:
        ...
        breakpoint = self.db.set_break(brk['fn'], brk['lno'], brk['temporary'], brk['cond'], brk['fun'])
    ...
    def do_unbreak(self, data):
        ...
        self.db.clear_break(brk['fn'], lno, brk['temporary'], brk['cond'], brk['fun'])

        self.db.send('BreakUnset|%s' % data)

    def do_breakpoints(self, data):
        self.db.send('Print|%s' % dump({'for': 'Breakpoints', 'result': self.db.breakpoints}))
    ...
    def do_complete(self, data):
        ...
        if not Interpreter:
            ...
            return
        try:
            script = Interpreter(source, [self.current_locals, self.get_globals()], **completion)
        ...
        if len(completions):
            ...
            base = completion.name[:len(completion.name) - len(completion.complete)]
            if len(base):
    ...
    def do_save(self, data):
        ...
        try:
            ...
            self.db.send('Echo|%s' % dump({'for': 'Error during save', 'val': str(e)}))
        else:
            self.db.send('Echo|%s' % dump({'for': 'Save succesful', 'val': 'Wrote %s' % fn}))
    ...
    def do_diff(self, data):
        if '?' not in data and '<>' not in data:
            ...
            return
        pretty = '?' in data
        expressions = [expression.strip() for expression in (data.split('?') if '?' in data else data.split('<>'))]
        strings = []
        for expression in expressions:
            try:
                strings.append(eval_(expression, self.get_globals(), self.current_locals))
            except Exception:
                self.fail('Diff', "Diff failed: Expression %s " "failed to evaluate to a string" % expression)
                return

        render = ((lambda x: self.db.better_repr(x, html=False) or self.db.safe_repr(x))) if pretty else str
        strings = [render(string) if not is_str(string) else string for string in strings]
        self.db.send(
            'RawHTML|%s' % dump(
                {
    ...
    def do_find(self, data):
        if ' in ' not in data and ' of ' not in data:
            self.fail(
                'Find', 'Find error', 'Syntax for find is: "key in expression" '
            ...
            'Print|%s' % dump(
                {
                    ...
                    'result':
                    'Found:\n%s' % '\n'.join(['%s: -> %s' % (k, escape(self.db.safe_repr(val))) for k, val in matches])
    ...
    def handle_exc(self):
        ...
        return '<a href="%d" class="inspect">%s: %s</a>' % (id(exc_info), escape(type_.__name__), escape(str(value)))
    ...
    def fail(self, cmd, title=None, message=None):
        ...
        self.db.send('Echo|%s' % dump({'for': escape(title or '%s failed' % cmd), 'val': message}))
