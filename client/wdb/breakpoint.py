...
class ConditionalBreakpoint(Breakpoint):
    ...
    def __repr__(self):
        return (super(ConditionalBreakpoint, self).__repr__() + ' under the condition %s' % self.condition)

    def __eq__(self, other):
        return super(ConditionalBreakpoint, self).__eq__(other) and self.condition == other.condition
...
class FunctionBreakpoint(Breakpoint):
    ...
    def breaks(self, frame):
        return (super(FunctionBreakpoint, self).breaks(frame) and frame.f_code.co_name == self.function)

    def __repr__(self):
        return (super(FunctionBreakpoint, self).__repr__() + ' in function %s' % self.function)

    def __eq__(self, other):
        return super(FunctionBreakpoint, self).__eq__(other) and self.function == other.function
