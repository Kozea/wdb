...
(function() {
  var $source, $traceback, backsearch, breakset, breakunset, cm, cm_theme, cmd_hist, code, create_code_mirror, cwd, die, dump, echo, ellipsize, execute, fallback, format_fun, get_mode, historize, init, last_cmd, log, make_ws, open, print, print_help, print_hist, register_handlers, searchback, searchback_stop, select, select_check, send, session_cmd_hist, start, started, stop, suggest, suggest_stop, termscroll, time, title, toggle_break, toggle_edition, trace, waited_for_ws, ws,
    _this = this,
  ...
  if (!this.indexedDB) {
    ...
    open = this.indexedDB.open('wdbdb', 3);
    ...
    open.onupgradeneeded = function(event) {
      ...
      if (event.oldVersion < 2) {
        db.createObjectStore("cmd", {
          keyPath: "name"
        });
      }
      return db.createObjectStore("file", {
        keyPath: "name"
      });
    };
  ...
  select_check = function(data) {
    return this.get('file')(data.name, (function(file) {
  ...
  select = function(data, reset_file) {
